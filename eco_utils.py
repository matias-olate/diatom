...
def get_feasible_points_from_batch_files(eco, file_dir,name_scenario,name_community_model, batch_size):
    ...
    batches = full_batches + (extra_batch > 0) 
    batches_first = [x*batch_size for x in range(batches)]


    batch_files = [os.path.join(file_dir, f"{file_prefix}_fpoints_{first}_{first + batch_size}.npy") for first in batches_first]
    if extra_batch > 0:
        batch_files[-1] = os.path.join(file_dir, f"{file_prefix}_fpoints_{batches_first[-1]}_{npoints}.npy")
...
def getHIERARCHICALclusters(dvector, k=20, lmethod='ward',criterion='maxclust', **kwards):
    ...
    #clusters = cut_tree(row_linkage, n_clusters=k).flatten()
    print(f"hierarchical, {k}, {row_linkage}, {clusters}")
    return k, clusters
