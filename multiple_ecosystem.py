...
class Ecosystem:
    ...
    @staticmethod
    def _no_coloring_plot(slice_points, lines=None, xlabel = None, ylabel= None, figsize=(11,12), s=16):
    ...
    def _categorical_coloring_plot(slice_points, slice_colors, parent_cmap, k, color_labels,  lines = None, 
                                   xlabel = None, ylabel= None, figsize=(11,12), s=16,shrink=1, saveFile='', pointsF=None, ax=None, title=None):
        if ax is None:
            myfig = plt.figure(figsize=figsize)            
        if lines is not None: 
            plt.gca().add_collection(lines)
        ...
        if pointsF is not None:
            ...
            ps = plt.scatter(slice_points[:,0],slice_points[:,1], c = slice_colors,cmap=cmap1, s=s,vmin=vmin,vmax=vmax)
       #for showing multiple plots without cbar
